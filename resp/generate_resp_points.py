...
def gaussian_reader(file_handle):
    """reads file_handle as a gaussian output file
...
class Atom:
    ...
    element_radii = {'H': 1.2, 'C': 1.7, 'N': 1.55, 'O': 1.52, 'S': 1.8, 'Se': 1.9}
...
with open(sys.argv[1]) as qm_file:
    ...
    for scale in range(14, 21, 2):
        ...
        pointses.append([])
        for atom in atoms:
            pointses[-1].extend(atom.radius_points(density=50, scale=scale/10, check_against=atoms))
    ...
    for color, points in zip(colors, pointses):
        ...
        ppl.scatter(points[:, 0], points[:, 1], c=points[:, 2])
    ...
    for axes in (0, 1, 2), (1, 2, 0), (0, 2, 1):
        plt.figure(figsize=(10, 10))
        ppl.scatter(grand_points[:, axes[0]], grand_points[:, axes[1]], c=grand_colors)
